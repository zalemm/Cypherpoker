/**
* @file Adapter for Bitcoin Cash blockchain interactions via the cryptoapis.io
*
* @version 0.5.0
* @author Patrick Bay
...
/**
* @class Bitcoin Cash adapter using cryptoapis.io's RPC API.
* @extends EventEmitter
*/
module.exports = class CryptoAPI extends BitcoinCashNative {
   ...
   *
   * @param {String} addressOrPath A cryptocurrency address or derivation path.
   * If a derivation path is supplied, a master wallet must exist in the
   * [server.]{@link CryptocurrencyHandler#server}<code>namespace.cp.wallets</code> from which the sub-wallet
   * will be derived.
   * @param {String} [APIType="bitcoincash"] The API endpoint to use for retrieving address information. Valid typs include:
   * "bitcoincash"
   *
   ...
   * the address or derived wallet. Refer to the <a href="https://docs.cryptoapis.io/#bch-address-info-endpoint">CryptoAPIs BCH Address
   * endpoint reference</a> for the properties of this object. The promise ends with a rejection if the API returns an error.
   *
   * @see https://docs.cryptoapis.io/#bch-address-info-endpoint
   */
   getBlockchainBalance(addressOrPath, APIType="bitcoincash", network=null) {
      ...
      var promise = new Promise((resolve, reject) => {
         ...
         var url = API.urls.cryptoapis.balance;
         if (network == null) {
         ...
         //api supports both legacy (base58) and Bitcoin Cash style addresses
         ...
         this.server.request({
           ...
           if (error) {
               ...
               resultObj.address = body.payload.legacy;
               resultObj.bchaddress = body.payload.address;
               resultObj.balance = this.convertDenom(body.payload.balance, "bitcoin", "satoshis");
               resultObj.unconfirmed_balance = 0;
               resultObj.final_balance = this.convertDenom(body.payload.balance, "bitcoin", "satoshis");
               resolve(resultObj);
           }
         });
      });
   ...
   */
   async getUTXOList(address, network="main") {
      var promise = new Promise((resolve, reject) => {
         var API = this.server.config.CP.API.bitcoincash;
         var url = API.urls.cryptoapis.gettxs;
         if (network == null) {
         network = API.default.network;
         }
         var token = this.server.config.CP.API.tokens.cryptoapis;
         ...
         this.server.request({
            ...
            if (error) {
               ...
               var utxoList = this.parseTransactionsList(body.payload, addressCash);
               resolve (utxoList);
            }
         });
      });
   ...
   * objects.
   *
   * @param {Array} rawTXList An indexed list of raw transactions containing
   * inputs and outputs associated with the <code>targetAddress</code>.
   * @param {String} targetAddress The address for which to generate the UTXO
   * output list from the <code>rawTXList</code>.
   ...
   */
   parseTransactionsList(rawTxList, targetAddress, genericUTXOFormat=true) {
      ...
      for (var count=0; count < rawTxList.length; count++) {
         ...
         var satoshiAmount = "";
         var scriptPubKey = "";
         for (var voutCount=0; voutCount < currentTx.txouts.length; voutCount++) {
            var currentTxOut = currentTx.txouts[voutCount];
            for (var addressCount=0; addressCount < currentTxOut.addresses.length; addressCount++) {
               var currentAddress = currentTxOut.addresses[addressCount];
               if (currentAddress == targetAddress) {
                  vout = voutCount;
                  satoshiAmount = this.convertDenom(currentTxOut.amount, "bitcoin", "satoshis");
                  scriptPubKey = currentTxOut.script.hex;
                  break;
               }
            }
         }
